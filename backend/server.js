...
// API 限流：按用户 ID（而非 IP），避免机房 NAT 同出口误限
...
const apiLimiter = rateLimit({
  ...
  skip: (req) => /\.(js|css|png|jpg|jpeg|gif|ico|woff2?|svg|ttf|eot)$/.test(req.path),
...
const authLimiter = rateLimit({
  ...
  message: { success: false, message: '操作过于频繁，请稍后重试' },
  validate: { xForwardedForHeader: false }
});
...
// 阶段一优化：考试核心接口按用户 ID 限流（避免机房 NAT 同 IP 误限）
const examApiLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 3000, // 50 人 × 每 30s 保存 = 每 15 分钟 1500 次，3000 留足余量
...
app.use('/api/exam-answers', examApiLimiter);
app.use('/api/exam-sessions', examApiLimiter);
app.use('/api/exam-enrollments', examApiLimiter);
