...
export function importExamFromPaper(paperId, name) {
  return request.post('/exams/import-from-paper', { paperId, name });
}
...
/** 同步所有考生客观题答案：从 exam_objective_answers 表提取到阅卷表 */
...
export async function exportObjectiveResults(examId, examName, withDetail = true) {
  const blob = await request.get(`/exams/${examId}/objective-results/export`, {
    params: withDetail ? { detail: '1' } : {},
    responseType: 'blob'
  });
...
// 获取从企业端同步过来的待导入批次列表
export function listImportBatches(status = 'INIT') {
  return request.get('/exam-imports/batches', { params: { status } });
}

// 执行某个导入批次到指定考试
export function importFromBatch(importBatchId, examId) {
  return request.post(`/exam-imports/batches/${importBatchId}/import`, { examId });
}

// 删除待导入批次（仅 INIT 状态）
export function deleteImportBatch(importBatchId) {
  return request.delete(`/exam-imports/batches/${importBatchId}`);
}

/** 面试成绩汇总（工作人员/企业端）：各考官总分、去高低后平均分、签字状态 */
