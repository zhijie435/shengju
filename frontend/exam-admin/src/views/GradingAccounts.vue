<template>
  <div>
    <el-page-header @back="$router.back()" title="系统子账号" />
    <div class="toolbar">
      <h2>系统子账号</h2>
...
function pickGradingListPayload(res) {
  if (!res || typeof res !== 'object') return { list: [], total: 0 };
  /** 拦截器可能返回整段 body { success, data:{list,total} }，也可能已被解包为 { list, total } */
  const inner = res.data != null && typeof res.data === 'object' && !Array.isArray(res.data) ? res.data : res;
  const list = Array.isArray(inner.list) ? inner.list : [];
  const total = Number(inner.total) >= 0 ? Number(inner.total) : 0;
  return { list, total };
}
...
async function loadAccounts() {
  ...
  try {
    ...
    const { list, total: t } = pickGradingListPayload(res);
...
async function handleSubmit() {
  const u = String(form.value.username || '').trim();
  const pwd = String(form.value.password || '');
  if (!u) {
  ...
  try {
    if (isEdit.value) {
      const updateData = { ...form.value, username: u };
