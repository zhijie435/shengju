<template>
  <div>
    ...
    <el-dialog
      ...
      destroy-on-close
    >
      <el-table :data="examineeList" stripe max-height="400" v-loading="examineeLoading">
    ...
    <!-- 客观题阅卷（执行阅卷 + 结果与导出 合并） -->
    <el-dialog
      v-model="objectiveGradingDialogVisible"
    ...
    >
      ...
      <template #footer>
        <el-button @click="objectiveGradingDialogVisible = false">关闭</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
// 客观题阅卷（合并结果）弹窗
const objectiveGradingDialogVisible = ref(false);
...
function openObjectiveGradingDialog(exam) {
  ...
  objectiveGradingDialogVisible.value = true;
...
async function doGradeObjective() {
  ...
  try {
    ...
    const gradedAnswers = results.reduce((sum, r) => sum + (Number(r.graded) || 0), 0);
    ElMessage.success(`阅卷完成：处理 ${results.length} 份，成功 ${ok} 份，失败 ${err} 份，判分 ${gradedAnswers} 题`);
...
function viewStatistics(exam) {
  if (isInterviewExam(exam)) {
    router.push({ path: `/exams/${exam.id}/interview-score-summary` });
  } else {
...
async function openExamineeStatus(exam) {
  ...
  try {
    const res = await getExamineeStatus(exam.id);
...
.empty-tip-small { padding: 12px; font-size: 12px; color: var(--el-text-color-secondary); }
</style>
