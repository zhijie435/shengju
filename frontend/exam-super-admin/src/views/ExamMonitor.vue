<template>
  <div>
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="考生监控" name="monitor">
        ...
        <p v-if="sessions.length" class="monitor-hint monitor-hint-live">
          说明：本页为<strong>视频分片监考</strong>（约每 6 秒一段 webm，含麦克风授权时带声音；点击放大可播放声音）。可使用「导出监控存档」打包留存全部片段。
        </p>
        <div class="monitor-grid">
          ...
          >
            <div class="monitor-card-header">
              <span class="name">{{ item.real_name || item.username || '考生' }}</span>
            ...
            <div class="monitor-card-body">
              <div class="video-cell" @click="openEnlarge(item, 'camera', item.camera)">
                ...
                <template v-if="item.camera?.file_path">
                  ...
                  <video v-else :key="'camera-'+(item.camera?.id||0)+'-'+chunkPollTick" :src="chunkUrl(item.camera)" muted playsinline autoplay loop class="monitor-media clickable" />
                </template>
              ...
              <div class="video-cell" @click="openEnlarge(item, 'side_camera', item.side_camera)">
                ...
                <template v-if="item.side_camera?.file_path">
                  ...
                  <video v-else :key="'side-'+(item.side_camera?.id||0)+'-'+chunkPollTick" :src="chunkUrl(item.side_camera)" muted playsinline autoplay loop class="monitor-media clickable" />
                </template>
                <div v-else class="no-video">暂无</div>
              </div>
              <div class="video-cell" @click="openEnlarge(item, 'screen', item.screen)">
                ...
                <template v-if="item.screen?.file_path">
                  ...
                  <video v-else :key="'screen-'+(item.screen?.id||0)+'-'+chunkPollTick" :src="chunkUrl(item.screen)" muted playsinline autoplay loop class="monitor-media clickable" />
                </template>
                <div v-else class="no-video">暂无</div>
              </div>
            </div>
    ...
    <el-dialog v-model="enlargeVisible" :title="enlargeTitle" width="90%" top="5vh" class="enlarge-dialog" @closed="enlargeChunk = null">
      <div v-if="enlargeChunk?.file_path" class="enlarge-content">
        ...
        <video v-else :src="chunkUrl(enlargeChunk)" muted playsinline autoplay controls class="enlarge-media" />
      </div>
    </el-dialog>
  </div>
</template>
...
import { listMonitorEvents, getLatestChunksByExam, downloadMonitorArchive } from '../api/monitor';
...
function openEnlarge(item, type, chunk) {
  ...
  enlargeTitle.value = `${item.real_name || item.username || '考生'} - ${labelMap[type] || type}`;
...
function chunkUrl(c) {
  ...
  return c?.id != null ? `${url}?v=${c.id}` : url;
}
...
function getSessionStatus(sessionId) {
  ...
  if (!s) return '-';
  const map = { pending: '待开始', ongoing: '进行中', submitted: '已交卷', abnormal: '异常' };
...
.monitor-media, .monitor-video { width: 100%; aspect-ratio: 4/3; background: #000; object-fit: contain; border-radius: 4px; max-height: 120px; }
