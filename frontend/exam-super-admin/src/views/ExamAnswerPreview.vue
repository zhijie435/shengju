      ...
      <template v-if="!loading && exam">
        ...
        <div class="exam-body" :class="{ 'a4-layout': !!previewHtml }">
          ...
          <div v-else class="question-list">
            ...
            <template v-for="major in majorQuestionsWithEnrichedSubs" :key="major.id || major.major_number">
              <div class="major-question-block">
                <div v-if="!isInterviewPreviewMode" class="major-question-header">
                  <strong>{{ major.major_number || '' }}、{{ major.question_type || '' }}</strong>
                  <span v-if="major.total_score != null" class="major-total-score">总分：{{ major.total_score }}分</span>
                  <span class="major-unified-setting">
                    <span>本大题答题方式：</span>
                    <el-select v-model="majorAnswerTypeTemp[major.id]" placeholder="选择" size="small" class="major-answer-type-select" clearable>
                      <el-option v-for="opt in ANSWER_TYPE_OPTIONS" :key="opt.value" :label="opt.label" :value="opt.value" />
                    </el-select>
                    <el-button type="primary" size="small" @click="applyMajorAnswerType(major.id, major.enrichedSubs.map(x => x.id))">应用到本大题</el-button>
                  </span>
                </div>
                <div v-if="!isInterviewPreviewMode && major.content_html && hasQuestionNumberInStem(major.content_html)" class="major-question-content" v-html="major.content_html" />
                <div v-for="q in major.enrichedSubs" :key="q.id" class="question-block">
              ...
              <template v-else-if="!isInterviewPreviewMode">
                <div v-for="subIdx in q.subItemCount" :key="subIdx" class="answer-box">
                  <label>作答({{ subIdx }})：</label>
                  <AnswerChoice v-if="q.answerType === 'choice' || q.answerType === 'judge'" v-model="previewAnswers[getQuestionKey(q, subIdx - 1)]" :options="q.options" :style="answerStyleConfig.choiceStyle" :disabled="false" />
                  <AnswerMultiChoice v-else-if="q.answerType === 'multichoice'" v-model="previewAnswers[getQuestionKey(q, subIdx - 1)]" :options="q.options" :style="answerStyleConfig.multichoiceStyle" :disabled="false" />
                  <AnswerBlank v-else-if="q.answerType === 'blank'" v-model="previewAnswers[getQuestionKey(q, subIdx - 1)]" :blank-count="q.blankCount" :style="answerStyleConfig.blankStyle" :disabled="false" />
                  <AnswerEssay v-else-if="q.answerType === 'essay'" v-model="previewAnswers[getQuestionKey(q, subIdx - 1)]" :word-count="q.wordCount" :disabled="false" />
                  <AnswerDrawing v-else-if="q.answerType === 'drawing'" v-model="previewAnswers[getQuestionKey(q, subIdx - 1)]" :width="q.drawingWidth || 500" :height="q.drawingHeight || 300" :disabled="false" />
                  <AnswerText v-else v-model="previewAnswers[getQuestionKey(q, subIdx - 1)]" :rows="4" :disabled="false" />
                </div>
              </template>
              <!-- 管理预览：每道小题的标准答案与解析（考生端不显示；面试时仅考官视图显示） -->
              <div v-if="(!isInterviewPreviewMode || previewViewMode === 'examiner') && (getStandardAnswer(q) || getExplanation(q))" class="answer-reference">
                <div v-if="getStandardAnswer(q)" class="ref-row">
                  <span class="ref-label">标准答案：</span>
                  <span class="ref-value">{{ getStandardAnswer(q) }}</span>
                </div>
                <div v-if="getExplanation(q)" class="ref-row">
                  <span class="ref-label">解析：</span>
                  <div class="ref-value ref-html" v-html="getExplanation(q)"></div>
                </div>
              </div>
                </div>
              </div>
...
import { getAnswerType, parseOptions, parseBlankCount, prependQuestionNumberTight, hasQuestionNumberInStem } from '../utils/questionParser';
...
const previewViewMode = ref('examiner');
...
/** 面试答题预览：不显示大题题干与答题框，不显示小题题号，显示标准答案与解析 */
...
/** 管理预览用：标准答案优先 standard_answer，否则 answer / answer_html 去 HTML */
function getStandardAnswer(q) {
  if (!q) return '';
  const s = (q.standard_answer != null && String(q.standard_answer).trim()) ? String(q.standard_answer).trim() : '';
  if (s) return s;
  if (q.answer != null && String(q.answer).trim()) return String(q.answer).trim();
  if (q.answer_html) return String(q.answer_html).replace(/<[^>]+>/g, '').trim();
  return '';
}

/** 管理预览用：解析使用 explanation 或 answer_analysis，可含 HTML */
function getExplanation(q) {
  if (!q) return '';
  const e = (q.explanation != null && String(q.explanation).trim()) ? String(q.explanation).trim() : '';
  if (e) return e;
  return (q.answer_analysis != null && String(q.answer_analysis).trim()) ? String(q.answer_analysis).trim() : '';
}
...
function enrichSubQuestion(sq) {
  ...
  const subItemCount = Math.min(Math.max((counts[sq.id] > 1 ? counts[sq.id] : 1) | 0, 1), 10);
  return {
    ...sq,
...
const majorQuestionsWithEnrichedSubs = computed(() => {
  ...
  return majors.map(major => {
    ...
    const enrichedSubs = subs.map(sq => {
      globalIndex++;
      const sqWithType = { ...sq, question_type: sq.question_type || major.question_type, majorType: major.question_type };
      const enriched = enrichSubQuestion(sqWithType);
      const displayNum = (overrides[sq.id] && String(overrides[sq.id]).trim()) ? String(overrides[sq.id]).trim() : String(globalIndex);
      return { ...enriched, globalDisplayNumber: displayNum };
    });
    return {
      ...major,
      enrichedSubs
  ...
  try {
    ...
    if (paperId) {
      ...
      subQuestions.value = majors.flatMap(m =>
        (m.subQuestions || []).map(s => ({ ...s, majorNumber: m.major_number, question_type: m.question_type }))
      );
    } else {
      subQuestions.value = [];
    }
...
.major-question-block { margin-bottom: 28px; }
.major-question-header { padding: 12px 16px; background: #f0f7ff; border: 1px solid #c6e2ff; border-radius: 8px 8px 0 0; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; }
.major-unified-setting { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.major-answer-type-select { width: 120px; }
.major-total-score { font-size: 13px; color: #606266; }
.major-question-content { padding: 16px 20px; background: #fafafa; border: 1px solid #e4e7ed; border-top: none; line-height: 1.8; font-size: 14px; }
.major-question-content :deep(img) { max-width: 100%; }
.question-block { margin-bottom: 0; padding: 20px; background: #fff; border: 1px solid #e4e7ed; border-top: none; }
.question-block:last-child { border-radius: 0 0 8px 8px; margin-bottom: 0; }
...
.answer-item-header { display: flex; align-items: center; gap: 12px; margin-bottom: 8px; flex-wrap: wrap; }
.answer-item-header label { margin: 0; font-weight: 500; flex-shrink: 0; }
...
.answer-reference { margin-top: 12px; padding: 12px; background: #f5f7fa; border: 1px solid #e4e7ed; border-radius: 6px; font-size: 13px; }
.answer-reference .ref-row { margin-bottom: 8px; }
.answer-reference .ref-row:last-child { margin-bottom: 0; }
.answer-reference .ref-label { font-weight: 600; color: #606266; margin-right: 6px; }
.answer-reference .ref-value { color: #303133; }
.answer-reference .ref-html :deep(img) { max-width: 100%; }
