      ...
      <el-tab-pane label="面试显示设置" name="display">
        ...
        <el-card shadow="never" class="mb-16 interview-flow-card">
          ...
          <el-alert type="info" :closable="false" show-icon class="mb-12">
            此处为<strong>整场面试的流程模式</strong>，保存后考生端叫号、候考与交卷逻辑会随之变化；与下方「试题显示」无关。
          </el-alert>
...
async function loadExaminers() {
  try {
    ...
    gradingAccounts.value = accountsRes.data?.list || accountsRes?.list || [];
...
function setChief(gradingAccountId) {
  chiefIdForRoomRef.value = gradingAccountId;
  const roomId = selectedRoomId.value;
  selectedExaminers.value = selectedExaminers.value.map((e) => {
    if (e.roomId !== roomId) return e;
    return { ...e, role: e.gradingAccountId === gradingAccountId ? 'chief' : 'interviewer' };
  });
}
