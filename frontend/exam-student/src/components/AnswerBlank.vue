  ...
  <div class="answer-blank" :class="styleClass">
    ...
    <template v-else>
      <div v-for="(_, i) in blankCount" :key="i" class="blank-item">
        ...
        <el-input
          :model-value="getBlank(i)"
          @update:model-value="v => updateBlank(i, v)"
          placeholder="请输入"
          clearable
          size="small"
          style="width: 180px"
...
const props = defineProps({
  ...
  blankCount: { type: Number, default: 1 },
...
const singleValue = computed(() =>
  props.blankCount <= 1 ? (props.modelValue || '') : ''
);
...
function updateSingle(v) {
  emit('update:modelValue', v);
}

function updateBlank(i, v) {
  const arr = Array.isArray(props.modelValue) ? [...props.modelValue] : [];
