...
// P2 优化：从 WebSocket 截图缓存按需拉取（替代 WS 广播 base64）
router.get('/screenshots/:sessionId/:seq', authenticate, requireRole('enterprise', 'admin', 'enterprise_reviewer'), (req, res) => {
  const data = getScreenshot(req.params.sessionId, parseInt(req.params.seq, 10));
  if (!data) return res.status(404).json({ success: false, message: '截图不存在或已过期' });
  // 支持 base64 或 buffer
  if (typeof data === 'string' && data.startsWith('data:')) {
    const mime = data.match(/^data:([^;]+)/);
    if (mime) res.setHeader('Content-Type', mime[1]);
    return res.send(Buffer.from(data.split(',')[1] || '', 'base64'));
  }
